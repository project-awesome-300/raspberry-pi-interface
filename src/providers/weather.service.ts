...
export class WeatherService {
   ...
   //current:CurrentWeather;
    constructor(private http:Http, private _app :AppService) {}
...
// }
    localWeather(lat:number, lon:number){
        return this.http.get(`http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=f9910e16c173698955d13abe28b6879a&units=metric&lang=${this._app.lang}`)
        .map((response:Response) => response.json());
    }

    forecastWeather(lat:number, lon:number){
        return this.http.get(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=f9910e16c173698955d13abe28b6879a&units=metric&lang=${this._app.lang}`)
        .map((response:Response) => response.json());
    }
